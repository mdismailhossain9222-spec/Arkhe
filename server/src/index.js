...
import { authRouter, enquiryRouter, supportRouter } from './routes.js'
...
app.get('/api/health', (_req, res) =>
  res.json({ ok: true, service: 'arkhe-api', time: new Date().toISOString() })
)
...
app.listen(config.port, () => {
  console.log(`ARKHE api listening on :${config.port} (${config.isProd ? 'production' : 'development'})`)
})
